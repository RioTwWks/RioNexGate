...
type TransportPresetDTO struct {
...
func stealthSettingsFromConfig(s *config.StealthConfig) StealthSettingsDTO {
	...
	dto.Fragmentation.Enabled = false
	dto.Fragmentation.Strategy = "serverhello"
	return dto
}
...
func applyStealthSettings(s *config.StealthConfig, dto StealthSettingsDTO) {
	...
	if dto.Presets.VisionReality.Port > 0 {
		s.Vision.Port = dto.Presets.VisionReality.Port
	}
	...
	if dto.Reality.Fingerprint != "" {
		s.Fingerprint = dto.Reality.Fingerprint
	}
	if dto.Reality.PrivateKey != "" {
		s.Reality.PrivateKey = dto.Reality.PrivateKey
	}
}
...
func (h *Handler) UpdateStealthSettings(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.core.Reload(); err != nil {
		writeError(w, http.StatusInternalServerError, "settings saved but core reload failed: "+err.Error())
		return
	}
	writeJSON(w, http.StatusOK, stealthSettingsFromConfig(&h.cfg.Core.Stealth))
}
...
func (h *Handler) TestStealthDest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		reqHTTP, err = http.NewRequestWithContext(r.Context(), http.MethodGet, url, nil)
		if err != nil {
			writeJSON(w, http.StatusOK, DestTestResponse{Reachable: false, Error: err.Error()})
			return
		}
	...
	writeJSON(w, http.StatusOK, DestTestResponse{
		Reachable:  true,
