...
- [x] Поддержать в конфиге фрагментацию **только первого пакета ServerHello**, не всех пакетов (снижение PPS-аномалии). *(частично: TLS inbound only)*
- [x] Параметры в `config.yaml` / UI: включение, размер/стратегия фрагментации (по документации актуальной версии Xray).
- [x] Значения по умолчанию — консервативные; предупреждение в UI при агрессивной фрагментации.
- [ ] **Ограничение upstream:** REALITY inbound + finalmask крашит Xray (v26.3.27–v26.7.28, [#6453](https://github.com/XTLS/Xray-core/issues/6453)).
...
- [x] Исследовать интеграцию: отдельный Docker-сервис / sidecar или внешний узел, управляемый из панели.
- [x] Модель `WireGuardPeer` или расширение `User` — ключи AmneziaWG, параметры обфускации (Jc, Jmin, Jmax, S1, S2, H).
- [x] API: выдача конфига/QR для AmneziaWG-профиля в подписке (отдельная строка или URI-схема).
- [x] Документация: когда использовать AWG vs Xray-пресеты.
